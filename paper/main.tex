...
\textbf{Square Attack (CE loss).} We deliberately run Square Attack with cross-entropy loss rather than its default margin loss. This is a methodological choice to isolate OTS's contribution: margin loss already provides implicit target tracking (Section~\ref{sec:drift}), which would confound OTS's effect. \textbf{Note:} this choice demonstrates OTS's \emph{mechanism}; practitioners using Square Attack should keep the default margin loss, which already provides the directionality that OTS adds. Budget is $N = 10{,}000$ iterations; the patch schedule depends on $N$, so all experiments use a fixed $N$ for comparability.
...
\caption{Success rate vs.\ query budget (100-image benchmark, all 5 standard models pooled). SimBA and Square Attack (CE) show clear OTS benefit (green tracks oracle in orange). Bandits (dotted) shows no separation, confirming that gradient estimation already provides directionality.}
...
Lock-in occurs early: SimBA switches at a median of 12.5 iterations (stability variant, $S=10$), Square Attack at median 141. With the recommended fixed-iteration switch, every run switches at exactly iteration $T$. Lock-match rates (the fraction of runs where OTS locks onto the class the untargeted attack would eventually reach) are moderate (35--75\% for random-search attacks). Mismatch does not imply failure: OTS closes nearly the entire gap between untargeted and oracle performance because it only needs \textit{a viable} class, not the oracle class. Across 1,021 paired successes, OTS produces fewer iterations than the oracle in 26.1\% of cases by locking onto a closer (non-oracle) class, suggesting that the trajectory-defined oracle is not always the fastest route to misclassification, making it a conservative reference point rather than a true upper bound. Detailed lock-in traces and lock-match analysis are in Appendix~\ref{app:secondary-figures}.
...
\begin{tabular}{@{}lllll@{}}
\toprule
Model & Method & Untarg. & Oracle & OTS \\
\midrule
...
A \textbf{target-class overlap analysis} (Figure~\ref{fig:target-overlap}) clarifies why. We compare OTS's locked class and the trajectory oracle class against the clean-image top-$K$ non-true classes on ResNet-50. SimBA locks onto the clean-image top-1 class in 100\% of runs, explaining its near-identical performance to $T = 0$: SimBA's small coordinate perturbations do not disrupt the initial ranking. Square Attack shows lower overlap (64\% at top-1, 81\% at top-10), because random-patch perturbations are more disruptive; this is why exploration provides a measurable gain for Square Attack but not SimBA. Notably, the trajectory oracle class falls \emph{outside} the clean-image top-1 in 19\% (SimBA) to 53\% (Square Attack) of cases, confirming that the oracle ceiling is trajectory-specific rather than geometry-optimal. A stronger geometry-based oracle (e.g., best-of-$K$ probes) would likely outperform the trajectory oracle, but it would also outperform OTS at proportional query cost; the practical comparison remains OTS vs.\ untargeted, where OTS's gains are clear.

Other target-selection heuristics exist (e.g., least-likely class, nearest class by embedding distance), but they require either full-vector knowledge before the attack begins or access to an embedding space. OTS's advantage is that it requires no \textit{a priori} target-class knowledge beyond what the attack's own trajectory reveals.

\textbf{Summary.} Opportunistic Target Selection demonstrates that the information needed to select an effective adversarial target is already latent in the attack's trajectory. By switching to a targeted objective at any small fixed iteration count, OTS eliminates the class drift that plagues probability-minimization and CE losses. On standard models with random-search attacks, OTS provides significant gains: up to +27 pp in success rate and 43\% in censored-mean iteration reduction on ResNet-50. On attacks with built-in directionality (Bandits, Square Attack with margin loss), OTS is redundant, reinforcing the margin-surrogate interpretation. On robust models, targeting is neutral due to a bimodal difficulty distribution.

\textbf{Limitations.}\label{sec:limitations}
(1)~A multi-seed validation (5 seeds $\times$ 100 images $\times$ 2 methods on ResNet-50, 3{,}000 runs) confirms OTS's effect is seed-independent: all 10 per-seed Wilcoxon tests remain significant after Bonferroni correction, with cross-seed standard deviation below 2~pp for every mode. The main benchmark uses a single seed for cost reasons, but the multi-seed check shows this does not affect conclusions.
(2)~Only two robust models were tested ($L_\infty$ adversarial training); other defenses (TRADES, MART, $L_2$) may differ.
(3)~All experiments use ImageNet ($K = 1000$), which maximizes drift potential; the benefit may diminish on datasets with fewer classes. However, $K = 1000$ is the standard benchmark for black-box attacks and represents the regime where class drift is most acute, making it the natural setting to evaluate OTS.
