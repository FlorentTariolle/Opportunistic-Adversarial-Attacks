...
\marginpar{\tiny\begin{turn}{90}\url{https://github.com/Tariolle/opportunistic-target-selection}\end{turn}}
...
\textbf{Images.} 100 images randomly sampled from ILSVRC2012 validation (seed 42). Each combination uses a single attack random seed (seed 0), yielding 4,500 runs on standard models. A separate multi-seed validation (5 seeds) confirms seed independence (Figure~10).
...
A \textbf{target-class overlap analysis} (Figure~8) clarifies why: SimBA locks onto the clean-image top-1 in 100\% of runs (small coordinate perturbations preserve the ranking), while Square Attack shows lower overlap (64\% at top-1) because random patches are more disruptive. This explains why exploration helps Square Attack but not SimBA.
