...
Usage:
    python analyze_theta.py --n-images 20 --budget 15000 --show
    python analyze_theta.py --n-images 2 --budget 500 --show   # Smoke test
"""
...
def interpolate_to_grid(all_trajectories, grid):
    ...
    for i, (iters, cos) in enumerate(all_trajectories):
        if len(iters) == 0:
            continue
        matrix[i] = np.interp(grid, iters, cos, left=np.nan, right=np.nan)
...
def main():
    ...
    # Look up oracle targets from winrate CSV
    ...
    for idx, path in enumerate(image_paths):
        ...
        if result['switch_iteration'] is not None:
            switch_iters.append(result['switch_iteration'])
